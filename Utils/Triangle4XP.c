...

{
  ...
  printf("triangle x%lx with orientation %ld:\n", (INT_PTR) t->tri,
         t->orient);
  ...
  if (printvertex == (vertex) NULL)
    printf("    Origin[%ld] = NULL\n", (t->orient + 1) % 3 + 3);
  else
    printf("    Origin[%ld] = x%lx  (%.12g, %.12g)\n",
...

{
  ...
  printf("subsegment x%lx with orientation %ld and mark %ld:\n",
  ...
  if (printvertex == (vertex) NULL)
    printf("    Segment origin[%ld] = NULL\n", 4 + s->ssorient);
  else
    printf("    Segment origin[%ld] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (INT_PTR) printvertex,
           printvertex[0], printvertex[1]);
  segdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Segment dest  [%ld] = NULL\n", 5 - s->ssorient);
  else
    printf("    Segment dest  [%ld] = x%lx  (%.12g, %.12g)\n",
...

{
  ...
  if (horrors == 0) {
    ...
    printf("  !! !! !! !! %ld abominations witnessed.\n", horrors);
  }
...

{
  ...
  "  By virtue of my perceptive intelligence, I declare the mesh Delaunay.\n");
    ...
    printf("  !! !! !! !! %ld obscenities viewed with horror.\n", horrors);
  }
  /* Restore the status of exact arithmetic. */
  b->noexact = saveexact;
}
...

{
  ...
  if (b->verbose > 2) {
    printf("  Triangulating %ld vertices.\n", vertices);
  }
  if (vertices == 2) {
    ...
    divconqrecurse(m, b, &sortarray[divider], vertices - divider, 1 - axis,
                   &innerright, farright);
    if (b->verbose > 1) {
      printf("  Joining triangulations with %ld and %ld vertices.\n", divider,
             vertices - divider);
    }
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        printf("ERROR:  Triangle %ld is missing vertex %ld in %s.\n",
    ...
    for (i = 0; i < m->insegments; i++) {
      ...
      if ((end1 < b->firstnumber) ||
        ...
        if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
          if (b->verbose>0) {
            printf("WARNING:  Endpoints of segment %ld are coincident in %s.\n",
                   b->firstnumber + i, polyfilename);
          }
        } else {
...

{
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      printf("ERROR:  Vertex %ld has no x coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
    x = (REAL) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      printf("ERROR:  Vertex %ld has no y coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
  ...
  if (*holes > 0) {
    ...
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        printf("ERROR:  Hole %ld has no x coordinate.\n",
               b->firstnumber + (i >> 1));
        triexit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        printf("ERROR:  Hole %ld has no y coordinate.\n",
               b->firstnumber + (i >> 1));
        triexit(1);
      } else {
    ...
    if (*regions > 0) {
      ...
      *rlist = regionlist;
      ...
      for (i = 0; i < *regions; i++) {
        ...
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          printf("Error:  Region %ld has no x coordinate.\n",
                 b->firstnumber + i);
          triexit(1);
        } else {
        ...
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          printf("Error:  Region %ld has no y coordinate.\n",
                 b->firstnumber + i);
          triexit(1);
        } else {
          regionlist[index++] = (REAL) strtod(stringptr, &stringptr);
        ...
        if (*stringptr == '\0') {
          printf(
            "Error:  Region %ld has no region attribute or area constraint.\n",
                 b->firstnumber + i);
          triexit(1);
        } else {
          regionlist[index++] = (REAL) strtod(stringptr, &stringptr);
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        if (trisym.tri == m->dummytri) {
          ...
          fprintf(outfile, "%4ld   %ld  %ld   %.17g  %.17g\n", vedgenumber,
...
{
  printf("\n   Statistics:\n\n");
  printf("   Input vertices: %ld\n", m->invertices);
  if (b->refine) {
    printf("   Input triangles: %ld\n", m->inelements);
  }
  if (b->poly) {
    printf("   Input segments: %ld\n", m->insegments);
    if (!b->refine) {
      printf("   Input holes: %ld\n", m->holes);
    }
  }
